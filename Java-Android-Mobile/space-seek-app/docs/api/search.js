...
function doSearch(request, response) {
  ...
  function searchIndex(indexArray, category) {
    ...
    $.each(indexArray, function (i, item) {
      ...
      var endOfName = category === "members" && input.indexOf("(", startOfName)
      > -1
...
$.widget("custom.catcomplete", $.ui.autocomplete, {
  ...
  _renderMenu: function (ul, items) {
    ...
    $.each(items, function (index, item) {
      if (item.category && item.category !== currentCategory) {
        ul.append(
            "<li class='ui-autocomplete-category'>" + categories[item.category]
            + "</li>");
    ...
    ul.append(
        "<li class='ui-static-link'><a href='" + pathtoroot + "search.html?q="
  ...
  _renderItem: function (ul, item) {
    ...
    if (item.category === "searchTags" && idx && idx.h) {
      if (idx.d) {
        div.html(label + "<span class='search-tag-holder-result'> (" + idx.h
...
$(function () {
  ...
  $("section[id] > :header, :header[id], :header:has(a[id])").each(
      function (idx, el) {
        ...
        if (id) {
          hdr.append(" <a href='#" + id + "' class='anchor-link' aria-label='"
  ...
  $(window).on("orientationchange", collapse).on("resize", function (e) {
    if (expanded && windowWidth !== window.innerWidth) {
      collapse();
    }
  });
